...
CREATE TABLE `debit_card` (
  `deb_card_id` int NOT NULL COMMENT 'TRIAL',
  `cvv` int NOT NULL COMMENT 'TRIAL',
  `valid_from` date NOT NULL COMMENT 'TRIAL',
  `valid_through` date NOT NULL COMMENT 'TRIAL',
  `trans_limit` int NOT NULL COMMENT 'TRIAL',
  `card_number` int NOT NULL COMMENT 'TRIAL',
  `holder_number` int NOT NULL COMMENT 'TRIAL',
  `status` varchar(255) NOT NULL COMMENT 'TRIAL',
  `deb_acc_id` int NOT NULL COMMENT 'TRIAL',
  `name` varchar(255) NOT NULL COMMENT 'TRIAL',
  `trial820` char(1) DEFAULT NULL COMMENT 'TRIAL',
  PRIMARY KEY (`deb_card_id`),
  UNIQUE KEY `debit_card_status_key` (`status`),
  UNIQUE KEY `debit_card_name_key` (`name`),
  CONSTRAINT `debit_card_deb_card_id_fkey` FOREIGN KEY (`deb_card_id`) REFERENCES `account` (`acc_id`)
